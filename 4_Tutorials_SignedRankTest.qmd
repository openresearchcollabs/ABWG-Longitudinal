...
The Wilcoxon signed-rank test was conducted to compare Job Satisfaction scores at two time points: T1 and T2. This test is a non-parametric alternative to the paired t-test and is suitable when the data might not meet the normality assumption. The test yielded a W statistic of W=`r round(test_result$statistic, 3)`. The associated p-value is p=`r round(test_result$p.value, 3)`. If this p-value is less than the common significance level of 0.05, we would reject the null hypothesis, indicating a statistically significant difference between the two time points.
