...
compute_descriptives <- function(data, event_name) {
  ...
  desc_df <- data.frame(
    Variable = c("Sex - Female", "Sex - Male or other", "Race - Asian", "Race - Black", "Race - Other/Mixed", "Race - White", "Age", "Weight"),
    Value = c(sex_desc, race_desc, age_desc, weight_desc)
  )
...
df_changing <- df %>%
  select(src_subject_id, eventname, anthroweightcalc)
...
df_wide <- df_wide %>% select(src_subject_id, site_id_l, sex, anthroweightcalc_0, anthroweightcalc_1, anthroweightcalc_2, anthroweightcalc_3, anthroweightcalc_4)
...
test_result <- wilcox.test(df_wide$anthroweightcalc_0, df_wide$anthroweightcalc_1, paired = TRUE)

```

**Descriptive Statistics for the Signed Rank Test result**
```{r model-checking}
# Compute xxxx for the signed rank test results
